import React from 'react';

function Hero(props) {
  return (
    <div className="inner">
      <div className="content-area">
        <h1 className="page-title">Hello 👋, I'm Kenny Krosky. I'm a front end developer enthusiastic about all things <span id="css-text">CSS</span> and <span id="js-text">JavaScript</span>.</h1>
        <p>Zombie ipsum reversus ab viral inferno, nam rick grimes malum cerebro. De carne lumbering animata corpora quaeritis. Summus brains sit​​, morbo vel maleficia? De apocalypsi gorger omero undead survivor dictum mauris.</p>
      </div>
    </div>
  )
}

export default Hero;